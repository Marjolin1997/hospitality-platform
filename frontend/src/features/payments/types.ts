...
export type CashSession={id:string;cash_register_id:string;base_currency:Currency;opening_cash:string;expected_cash_live?:string;expected_cash?:string|null;counted_cash?:string|null;cash_difference?:string|null;status:'open'|'closed';opened_at:string;closed_at?:string|null;register?:CashRegister};
...
export type PaymentRefund={id:string;payment_id:string;invoice_credit_note_id?:string|null;amount:string;amount_base:string;currency:Currency;base_currency:Currency;reason:string;status:string;refunded_at:string};
