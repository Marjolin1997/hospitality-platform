...
type CreditNoteState={id:string;number:string;status:string;currency:Currency;grand_total:string;reason:string};
type Props = { orderId: string; grandTotal: number; paidTotal: number; baseCurrency: Currency; payments: Payment[]; allowCollect: boolean; allowRefund: boolean; issuedInvoiceNumber?: string|null; invoiceCreditNote?:CreditNoteState|null };
...
export function PaymentPanel({ orderId, grandTotal, paidTotal, baseCurrency, payments, allowCollect, allowRefund, issuedInvoiceNumber, invoiceCreditNote }: Props) {
  ...
  const creditedRefundBase=invoiceCreditNote?payments.flatMap(p=>p.refunds??[]).filter(r=>r.status==='completed'&&r.invoice_credit_note_id===invoiceCreditNote.id).reduce((sum,r)=>sum+Number(r.amount_base),0):0;
  const creditRemaining=invoiceCreditNote?Math.max(Number(invoiceCreditNote.grand_total)-creditedRefundBase,0):Infinity;
  const refundBase=selectedRefund?refundValue*Number(selectedRefund.exchange_rate):0;
  const creditRefundReady=!invoiceCreditNote||refundBase<=creditRemaining+.00005;
  const refundValid=Boolean(selectedRefund)&&Number.isFinite(refundValue)&&refundValue>0&&refundValue<=Number(selectedRefund?.refundable??0)+.00005&&refundReason.trim().length>=3&&creditRefundReady;
  const drawerExpected=Number(selectedSession?.expected_cash_live??selectedSession?.opening_cash??0);
  const cashRefundReady=!selectedRefund||selectedRefund.method!=='cash'||Boolean(selectedSession)&&drawerExpected+.00005>=refundValue;
  ...
  return <section className="payment-panel">
    ...
    <button type="button" className="primary-action full" disabled={collect.isPending||quote.isLoading||!validAmount||!validTendered||(method === 'cash' && !selectedSession)} onClick={submit}>{collect.isPending ? 'Processing payment…' : `Collect ${format(validAmount?numericAmount:0,currency)}`}</button></>}
    ...
    {allowRefund&&issuedInvoiceNumber&&!invoiceCreditNote&&<div className="refund-panel"><div className="panel-heading"><div><h3>Refund requires invoice correction</h3><p>Invoice {issuedInvoiceNumber} has already been issued. Create a credit note from Invoices before returning money.</p></div></div></div>}
    {allowRefund&&invoiceCreditNote&&creditRemaining<=.00005&&<div className="refund-panel"><div className="panel-heading"><div><h3>Credit note fully refunded</h3><p>{invoiceCreditNote.number} has no remaining refund authorization.</p></div></div></div>}
    {allowRefund&&invoiceCreditNote&&creditRemaining>.00005&&<div className="payment-quote"><span>Credit note <strong>{invoiceCreditNote.number}</strong></span><small>{format(creditRemaining,baseCurrency)} remains authorized for refund · {invoiceCreditNote.reason}</small></div>}
    {allowRefund&&(!issuedInvoiceNumber||(invoiceCreditNote&&creditRemaining>.00005))&&<div className="refund-panel"><div className="panel-heading"><div><h3>Refund payment</h3><p>Refund against the original payment and exchange-rate snapshot.</p></div></div>{refundablePayments.length===0?<p className="field-hint">No refundable completed payments.</p>:<><label><span>Payment</span><select value={refundPaymentId} onChange={e=>{const id=e.target.value;setRefundPaymentId(id);const p=refundablePayments.find(x=>x.id===id);setRefundAmount(p?String(p.refundable):'')}}><option value="">Select payment</option>{refundablePayments.map(p=><option key={p.id} value={p.id}>{p.method} · {format(p.refundable,p.currency)} refundable</option>)}</select></label>{selectedRefund&&<><label><span>Refund amount ({selectedRefund.currency})</span><input type="number" min="0.01" max={selectedRefund.refundable} step="0.01" inputMode="decimal" value={refundAmount} onChange={e=>setRefundAmount(e.target.value)}/></label><label><span>Reason</span><textarea minLength={3} maxLength={500} value={refundReason} onChange={e=>setRefundReason(e.target.value)} placeholder="Required for audit trail"/></label>{selectedRefund.method==='cash'&&<label><span>Refund from register</span><select value={cashSessionId} onChange={e=>setCashSessionId(e.target.value)}><option value="">Select open register</option>{(sessions.data??[]).map(s=><option key={s.id} value={s.id}>{s.register?.name??'Register'} · {format(Number(s.expected_cash_live??s.opening_cash),s.base_currency)}</option>)}</select></label>}{selectedRefund.method==='cash'&&!selectedSession&&<p className="error-state">Select an open cash register for this refund.</p>}{selectedRefund.method==='cash'&&selectedSession&&drawerExpected+.00005<refundValue&&<p className="error-state">This drawer has {format(drawerExpected,selectedSession.base_currency)} expected cash, below the requested refund.</p>}{invoiceCreditNote&&refundBase>creditRemaining+.00005&&<p className="error-state">This refund converts to {format(refundBase,baseCurrency)}, above the {format(creditRemaining,baseCurrency)} remaining on {invoiceCreditNote.number}.</p>}{refund.isError&&<p className="error-state">Refund could not be completed. Check refundable balance and cash session.</p>}<button type="button" className="danger-action full" disabled={!refundValid||refund.isPending||!cashRefundReady} onClick={submitRefund}>{refund.isPending?'Processing refund…':`Refund ${format(refundValid?refundValue:0,selectedRefund.currency)}`}</button></>}</>}</div>}
  </section>;
}
