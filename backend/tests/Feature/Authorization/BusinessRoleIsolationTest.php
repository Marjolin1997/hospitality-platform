...
test('operational role templates enforce least privilege contracts', function (): void {
    ...
    expect(rbacRolePermissionKeys($owner))->toBe(Permission::query()->orderBy('key')->pluck('key')->all())
        ...
        ->and(rbacRolePermissionKeys($manager))->not->toContain('fiscalization.manage', 'fiscalization.activate_production')
        ...
        ->and(rbacRolePermissionKeys($finance))->not->toContain('fiscalization.manage', 'fiscalization.activate_production')
        ->and(rbacRolePermissionKeys($owner))->toContain('fiscalization.manage', 'fiscalization.issue', 'fiscalization.retry', 'fiscalization.activate_production');
});
