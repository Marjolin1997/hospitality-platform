...
class OrderOperationsController extends Controller
{
    public function __construct(
        private readonly OrderOperationsService $operations,
        private readonly OrderTransferService $transfers,
    ) {}
    ...
    public function split(SplitOrderRequest $request, Order $order): JsonResponse
    {
        return response()->json([
            'data' => $this->transfers->split(app(Business::class), $request->user(), $order, $request->validated()),
        ], 201);
    }

    public function merge(MergeOrderRequest $request, Order $order): JsonResponse
    {
        return $this->response($this->transfers->merge(app(Business::class), $request->user(), $order, $request->validated()));
    }
