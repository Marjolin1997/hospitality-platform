...
class PaymentController extends Controller
{
    ...
    {
        return response()->json(['data' => CashRegister::query()->forBusiness(app(Business::class))->where('is_active', true)->with('location')->orderBy('name')->get()]);
    }
    ...
    {
        return response()->json(['data' => $service->execute(app(Business::class), $request->user(), $request->validated())->load('register')], 201);
    }

    public function movement(StoreCashMovementRequest $request, string $session, RecordCashMovement $service): JsonResponse
    {
        $record = CashSession::query()->forBusiness(app(Business::class))->whereKey($session)->firstOrFail();
        return response()->json(['data' => $service->execute(app(Business::class), $request->user(), $record, $request->validated())], 201);
    }

    public function closeSession(CloseCashSessionRequest $request, string $session, CloseCashSession $service): JsonResponse
    {
        $record = CashSession::query()->forBusiness(app(Business::class))->whereKey($session)->firstOrFail();
        return response()->json(['data' => $service->execute(app(Business::class), $request->user(), $record, $request->validated())]);
    }
